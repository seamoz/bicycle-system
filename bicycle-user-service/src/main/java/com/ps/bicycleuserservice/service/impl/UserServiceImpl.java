...
import com.ps.bicycleuserservice.mapper.VerifyMapper;
...
public class UserServiceImpl implements UserService {
    ...
    /**
     * redis 忘记密码邮箱地址或手机号验证码存缓 的key
     * */
    private static final String userEmailOrPassword = "emailOrPassword:";
    ...
    @Override
    public Result logInByPhoneAndVerify(Verify verify) {
        ValueOperations <String, String> operations = template.opsForValue();
        String code = operations.get("phone_" + verify.getPhone());
        if (null == code) {
            return new Result("验证码已过期", 101);
        }
    ...
    @Override
    public Result sendVerify(Verify verify) {

        String code = CreateCode.createCode();
        System.out.println("------------verify:" + code + "---------------");
        ValueOperations <String, String> operations = template.opsForValue();
        operations.set("phone_" + verify.getPhone(), code, 90, TimeUnit.SECONDS);
    ...
    public Result addEmail(int id, String email) {
        ...
        if (!verify.getVerify().equals(verify)) {
    ...
    public Message<String> resetPassword(String key, String verificationCode) {
        ...
        boolean judeg = judgeCode(userEmailOrPassword+key,verificationCode);
        if(!judeg){
            message.setCode(0);
    ...
    public Message<String> setPassword(String key,String password, Integer index) {
        ...
        //开始设置密码
        String newPassword = MD5Encryption.encryption(password);
        if(index == 1){
            userMapper.updatePasswordByPhone(key,newPassword);
        }else{
            userMapper.updatePasswordByEmail(key,newPassword);
        }
